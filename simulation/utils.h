        ...
        for (int j = 1; j < particles.size(); j++)
        {
            ...
            else
            {
                if (particles[i].getDX() > 0)
                {
                    ...
                    if(timeToEvent){
                        ParticleCollideEvent(particles[i],WALL_X_POSTIVO,timeToEvent);
                    }
                }
                else if (particles[i].getDX() < 0)
                {
...
void advanceParticleToNextEvent(std::vector <Particle> &particles,ParticleCollideEvent const &nextEvent ){
